...
export function RevisionCalendarHeatmap({ sessions }: RevisionCalendarHeatmapProps) {
  ...
  return (
    <div className="bg-zinc-900/90 border border-white/15 rounded-3xl p-6 space-y-4 text-left select-none shadow-2xl relative overflow-hidden">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
        ...
        <div className="flex items-center gap-3 font-mono text-xs">
          <div className="bg-zinc-950/60 border border-white/10 px-3 py-1.5 rounded-xl flex items-center gap-1.5">
            <Flame className="w-3.5 h-3.5 text-amber-400" />
            <span className="text-zinc-300"><strong>{activeDays30d}</strong>/30 Active Days</span>
          </div>
          <div className="bg-indigo-950/60 border border-indigo-500/40 px-3 py-1.5 rounded-xl text-indigo-300 flex items-center gap-1.5 shadow-sm">
      ...
      {/* 30-Day Grid */}
      <div className="bg-zinc-950/70 border border-white/10 rounded-2xl p-4 space-y-3">
