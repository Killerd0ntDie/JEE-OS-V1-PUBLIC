...
export function CustomMissionHistoryModal({ isOpen, onClose }: CustomMissionHistoryModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} zIndex={100} className="w-full max-w-2xl bg-black/45 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] border border-white/10">
      {/* Header */}
      <div className="flex items-center justify-between p-5 border-b border-zinc-900/80 bg-zinc-900/40">
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors cursor-pointer select-none active:scale-95"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      {/* Body */}
      <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar max-h-[60vh] text-left">
            ...
            >
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  className="w-9 h-9 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-400 hover:text-indigo-400 hover:bg-indigo-500/10 hover:border-indigo-500/30 flex items-center justify-center transition-all duration-150 active:scale-[0.95] cursor-pointer relative z-10"
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-400 hover:text-red-400 hover:bg-red-500/10 hover:border-red-500/30 flex items-center justify-center transition-all duration-150 active:scale-[0.95] cursor-pointer relative z-10"
