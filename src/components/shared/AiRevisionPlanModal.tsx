...
export function AiRevisionPlanModal({ isOpen, onClose }: AiRevisionPlanModalProps) {
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      // Deterministic fallback plan if offline
      const fallbackPlan = {
        summary: `Strategic ${selectedDays}-Day Sprint designed for ${settings.dreamIit || 'IIT'} (${settings.targetYear}). Focuses on high-yield topic reinforcement and backlog elimination.`,
        days: Array.from({ length: selectedDays }).map((_, dIdx) => ({
          dayNumber: dIdx + 1,
          title: dIdx === 0 ? 'Mechanics & Physical Chemistry Sprint' : dIdx === 1 ? 'Calculus & Organic Mechanisms' : 'Electrodynamics & Inorganic Review',
          tasks: [
            { title: 'Solve 15 High-Yield PYQs', chapter: 'Core Topics', type: 'Solve PYQs', subject: 'physics', durationMinutes: 60 },
            { title: 'Active Formula Recall Drill', chapter: 'Formulas', type: 'Revise Formulas', subject: 'chemistry', durationMinutes: 45 },
            { title: 'Mistake Notebook Autopsy', chapter: 'Weak Spots', type: 'Review Mistakes', subject: 'maths', durationMinutes: 45 }
          ]
        }))
      };
    ...
    >
      ...
      {!generatedPlan && !isGenerating && (
        <div className="space-y-6 flex-1 overflow-y-auto pr-1">
          <div className="bg-zinc-800/60 border border-white/10 rounded-2xl p-5 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-bold text-sm text-white">3-Day Emergency Sprint</span>
                  <Calendar className="w-4 h-4 text-indigo-400" />
                </div>
                <span className="text-xs text-zinc-400 font-sans">High-yield fix for urgent bottlenecks</span>
              </button>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-bold text-sm text-white">7-Day Master Plan</span>
                  <Calendar className="w-4 h-4 text-indigo-400" />
                </div>
                <span className="text-xs text-zinc-400 font-sans">Full-syllabus revision & balance</span>
              </button>
            </div>
          </div>

          {errorMsg && (
            <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-red-300 text-xs font-mono">
              {errorMsg}
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-mono text-sm font-bold uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer"
          >
      ...
      {generatedPlan && !isGenerating && (
        <div className="space-y-6 flex-1 overflow-y-auto pr-1">
          <div className="bg-zinc-800/80 border border-white/15 rounded-2xl p-4">
          ...
          <div className="space-y-4">
            {generatedPlan.days?.map((day: any) => (
              <div key={day.dayNumber} className="bg-zinc-850/80 border border-white/15 rounded-2xl p-5 space-y-3">
                ...
                <div className="space-y-2">
                  {day.tasks?.map((t: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900/80 border border-white/10 text-xs">
                        <div className="space-y-0.5">
                          ...
                          <span className="text-[10px] text-zinc-400 font-mono">{t.chapter} • {t.type}</span>
                        </div>
                        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-3 bg-zinc-800 hover:bg-zinc-700 border border-white/10 text-zinc-300 rounded-2xl font-mono text-xs font-bold uppercase transition-colors cursor-pointer"
            >
              Re-configure
            </button>
            <button
              onClick={onClose}
              className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-mono text-xs font-bold uppercase shadow-lg shadow-indigo-600/30 transition-colors cursor-pointer"
            >
              Done
            </button>
          </div>
